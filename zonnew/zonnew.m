...
if isempty(ZONNEWarch)
	ZONNEWarch = {now,varargin};
else
	ZONNEWarch{end+1,1} = now;
...
if ~isempty(varargin)
	...
	elseif iscell(varargin{1})
		...
		for i = 1:2:length(opties)
			B = strcmpi(opties{i},sOpties);
			if ~any(B)
				error('optie "%s" is onbekend.',opties{i})
			end
			Shuis.(sOpties{B}) = opties{i+1};
			if any(strcmpi(opties{i},{'DsetProj','PsetProj'}))
				bAutoSpiegelOr = 1;
			end
			if any(strcmpi(opties{i},	...
