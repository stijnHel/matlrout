function a=leesscan(c,nr,SOFnr,nROWmax)
% CJPEG/LEESSCAN - Leest JPEG-scan
%     a=leesscan(c[,nr[,SOFnr]])
...
if length(i)>1
	warning('LEESSCAN:MultiSOF','!!!meerdere SOF-markers')
	i=i(1);
elseif isempty(i)
...
case 'SOF_0'
	% OK, "gewoon"
otherwise
	error('!!andere frames dan "gewone" zijn nog niet voorzien (%s)',c.data(i).markerName);
end
d=double(c.data(i).data);
nbit=d(1);
if nbit~=8
	warning('LEESSCAN:No8bits','Only tested for 8 bits (here: %d bits)!',nbit)
end
Y=d(2:3)*[256;1];
X=d(4:5)*[256;1];
ncomp=d(6);
...
while ~SOSgevonden
	...
	case 'DHT'
		...
		for i=1:length(H1)
			if H1(i).type<16
				if H1(i).type>=4
					warning('LEESSCAN:MoreThan4DChuffs'	...
						,'!!!!!????meer dan 4 dc-huffman-tabellen????')
				end
				if isempty(Hdc)
					Hdc=H1([i i i i]);
				else
			...
			elseif floor(H1(i).type/16)>1
				...
				if j>4
					warning('LEESSCAN:MoreThan4AChuffs'	...
						,'!!!!!????meer dan 4 ac-huffman-tabellen????')
				end
				if isempty(Hac)
					Hac=H1([i i i i]);
				else
	...
	case 'SOS'
		...
		end		% end was missing, just added but are more lines missing???
	end	% switch
	iMarker=iMarker+1;
	...
	for iCOL=1:nCOL_MCU
		...
		for i=1:n
			for j=1:prod(COMP(comp(i,1),2:3))
				...
				Ydct(:)=xDCT(iZigZag);
				%Xdct=round(min(255,max(0,idct2(Ydct)+128)));	% requires im proc toolbox
				Xdct=round(min(255,max(0,idct(idct(Ydct).').'+128)));
				k=k+1;
