function [A,namen,dim,BB,gegs,str,err] = leesmusi(naam,start,Lengte,ongeschaald,kanalen,i1)
% LEESMUSI - leest data van het MUSICS-systeem.
%    [A,namen,dim,BB,gegs,str,err] = leesmusi(naam,start,lengte,ongeschaald,kanalen)
...
if ~isstr(naam)
	...
	elseif naam<0
		[A,namen,dim,BB,gegs,str,err]=leesmusi(sprintf('%07d',-naam));
		return
	else
		error('leesmusi(0) is niet mogelijk');
	end
...
elseif isdir([EVDIR naam])
	...
	for i=1:length(direc)
		if ~direc(i).isdir
			[a1,n1,d1,BB,g1,str,err]=leesmusi([naam '\' direc(i).name]);
			if isempty(gegs)
				gegs=g1;
			else
			...
			if nieuwe
				...
				if size(a1,1)>size(A{j},1)
                    ...
                    fprintf('sommige kanalen werden aangevuld met nullen omdat ze niet allemaal even lang waren\n');
                    A{j}=[A{j};zeros(size(a1,1)-size(A{j},1),size(A{j},2))];
    				A{j}=[a1(:,1) [A{j}(:,2:end);zeros(size(a1,1)-size(A{j},1),size(A{j},2)-1)] a1(:,2:end)];
				elseif size(a1,1)<size(A{j},1)
                    fprintf('sommige kanalen werden aangevuld met nullen omdat ze niet allemaal even lang waren\n');
					%fprintf('%d van de %d meetpunten weggehaald\n',size(A{j},1)-size(a1,1),size(a1,1));
    				A{j}=[A{j} [a1(:,2:end);zeros(size(A{j},1)-size(a1,1),size(a1,2)-1)]];
                else
    				A{j}=[A{j} a1(:,2:end)];
                end
	...
	if (length(A)==1)
		...
		[dts,i]=sort(dts);
		if length(A)>2
			errordlg('Meer dan twee sample-tijden zijn in DaqView (nog) niet toegestaan.  De snelste twee worden ingelezen.','LEESMUSI-error','modal');
			%!!!!!!!!daqview-melding gebruikt!!!!
			daqerror('Meer dan twee sample-tijden in daqview/leesmusi ingelezen',-1)
			dts=dts(1:2);
			namen=namen{i(1:2)};
			dim=dim{i(1:2)};
		end
		BB=A{i(2)};
		A=A{i(1)};
		namen=addstr(namen{i(1)},namen{i(2)}(2:end,:));
		dim=addstr(dim{i(1)},dim{i(2)}(2:end,:));
	end
...
while ~feof(fid)
	...
	if isempty(findstr(nietlezen,s))
		sf=fread(fid,lengte,'uchar');
		sf=setstr(sf');
	end
	if s(1)=='C'
		if s(2)=='F' % file format
			if versie~=2
				fclose(fid);
		...
		elseif s(2)=='B' % group definition
			...
			mogelijkheden='CG,Cb,';
		elseif s(2)=='T' % definition of text
			if versie~=1
				fclose(fid);
		...
		elseif s(2)=='G' % definition of data field, start of a group
			if versie~=1
				fclose(fid);
		...
		elseif s(2)=='C' % start of a component
			if versie~=1
				fclose(fid);
			...
			ad=lees1get(sf);
			if info
				fprintf('comp #%2d index : %1d, ',compIndex);
			end
			if info
				if ad==1
					fprintf('analoog\n');
			...
			mogelijkheden='CD,NT,CP,Cb,';
		elseif s(2)=='P' % packet information
			if versie~=1
				fclose(fid);
				error('versie 1 verwacht voor scaling')
			end
			...
			if info
				...
				elseif numberFormat==2
					fprintf('signed byte, ');
				elseif numberFormat==3
					fprintf('unsigned short, ');
				elseif numberFormat==4
					fprintf('signed short, ');
				elseif numberFormat==5
					fprintf('unsigned long, ');
				elseif numberFormat==6
					fprintf('signed long, ');
				elseif numberFormat==7
					fprintf('float, ');
				elseif numberFormat==8
					fprintf('double, ');
				elseif numberFormat==11
		...
		elseif s(2)=='b' % buffer description
			if versie~=1
				fclose(fid);
				error('versie 1 verwacht voor scaling')
			end
			...
			for i=1:numberBufferInKey
				...
				if indexSamplesKey~=1
					fclose(fid);
					error('Meerdere CS-keys zijn nog niet geimplementeerd');
				end
				if info
			...
			mogelijkheden='CS,CR,Cb,';
		elseif s(2)=='R' % value range
			if versie~=1
				fclose(fid);
				error('versie 1 verwacht voor scaling')
			end
			...
			mogelijkheden='CN,Cb,';
		elseif s(2)=='N' % name, comment about channel
			if versie~=1
				fclose(fid);
			...
			mogelijkheden='NU,Cb,CG,CB,CT,CS,';
		elseif s(2)=='S' % samples
			if versie~=1
				fclose(fid);
			...
			if isempty(ts)
				...
				if max(lens)~=min(lens)
					fclose(fid);
					error('Toch niet allemaal even lang');
				end
				A=zeros(min(Lengte,lens(1)-start),size(namen,1));
				k=0;
				for i=1:size(DataGegs,1)
					nb=DataGegs(1,iBytes);
					fseek(fid,filepos+DataGegs(i,iOffsetBufferInSamplesKey)+start*nb+DataGegs(i,iOffset),'bof');
					if DataGegs(i,iNumberFormat)==1
						typ='uint8';
					elseif DataGegs(i,iNumberFormat)==2
						typ='int8';
					elseif DataGegs(i,iNumberFormat)==3
						typ='uint16';
					elseif DataGegs(i,iNumberFormat)==4
						typ='int16';
					elseif DataGegs(i,iNumberFormat)==5
						typ='uint32';
					elseif DataGegs(i,iNumberFormat)==6
						typ='int32';
					elseif DataGegs(i,iNumberFormat)==7
						typ='single';
					elseif DataGegs(i,iNumberFormat)==8
						typ='double';
					elseif DataGegs(i,iNumberFormat)==11
						typ='uint16';
					elseif DataGegs(i,iBytes)==1
						typ='int8';
					elseif DataGegs(i,iBytes)==2
						typ='int16';
					elseif DataGegs(i,iBytes)==4
						typ='int32';
					elseif DataGegs(i,iBytes)==8
						typ='int64';
					else
						fclose(fid);
	...
	elseif s(1)=='N'
		...
		elseif s(2)=='D' % display properties
