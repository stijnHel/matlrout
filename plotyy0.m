function [hy2l,ax,pl]=plotyy0(arg1,arg2,arg3,arg4,arg5,arg6,arg7)
% PLOTYY0  Plot graphs with Y tick labels on left and right side.
%         PLOTYY0(x1,y1,x2,y2) plots y1 vs. x1 with y-axis labeling
%         on left, and plots y2 vs. x2 with y-axis labeling on right.
%
%         PLOTYY(x1,y1,linopts1,x2,y2,linopts2) draws lines according
%         to (optionally) specified line types.  LINOPTS* is a string
%         specified according the line options for PLOT.
%
%         [hy2l] = PLOTYY(x1,y1,x2,y2) returns the handle of the text
%         object used for y2label.
%
%         [hy2l,ax,pl] = PLOTYY(x1,y1,x2,y2) additionally returns the
%         handles of the two axes created by plotyy in ax and the
%         handles of the lines created by plotyy in pl.
%
%         PLOTYY(x1,y1,x2,y2,options) allows for an options
%         vector as follows:
%
...
%         For example,
%
%               plotyy(x1,y1,'r-',x2,y2,'g:',[0 1 0 1.5 3 nan nan 0 10])
%
...
%         of 0 to 10.
%
%         Note:  Using PLOT to replace axes created by PLOTYY will not
%         replace the second y-axis.  Use PLOTYY with empty x2 and y2
%         vectors, i.e., plotyy(x1,y1,[],[]).
%
%         See also TITLE, XLABEL, YLABEL, Y2LABEL, SUBPLOT.
%!!!!old version = not compatible with newer matlab (?>7.0)
...
if strcmp(get(gca,'Tag'),'plotyy-axes1')
