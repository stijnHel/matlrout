...
%MORSE - Convert text into Morse code audio. (aangepaste versie)
...
if rem(length(tdot),2)
	tdot(end+1)=tdog(end)+tdot(2);
end
tdash = 0:1/8192:dashlen;
if rem(length(tdash),2)
	tdash(end+1)=tdash(end)+tdash(2);
end
dot = sin(tdot*4000);
dash = sin(tdash*4000);
ldot = length(dot);
